...
// Scene Forge Price IDs — use env vars, no hardcoded fallbacks
const MONTHLY_PRICE_ID = process.env.STRIPE_PRICE_ID_MONTHLY || "";
const YEARLY_PRICE_ID = process.env.STRIPE_PRICE_ID_YEARLY || "";

const TRIAL_DAYS = 7;

// Initialize Stripe lazily — only when a key is available
...
export function isTrialActive(user: any): boolean {
  ...
  const trialStart = user.trial_started_at || user.trialStartedAt || user.created_at || user.createdAt;
...
export function canAccessFeatures(user: any): boolean {
  return isCreator(user) || hasActiveSubscription(user) || isTrialActive(user);
}
...
export function registerStripeRoutes(app: Express) {
  ...
  app.get("/api/subscription/status", (req: Request, res: Response) => {
    ...
    res.json({
      status: isCreator(user) ? "creator" : (user.subscription_status || "trial"),
  ...
  app.post("/api/stripe/create-checkout", async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!priceId) {
        return res.status(500).json({ message: "Stripe price IDs not configured" });
      }
  ...
  // NOTE: This route needs raw body — must be registered BEFORE express.json in index.ts
